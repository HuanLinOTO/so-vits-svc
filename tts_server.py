...
async def tts(spk, req: RequestBody):
    ...
    if os.path.exists(audio_path):
        return {
            "status": "success",
            "src": "/wav/" + str(unique_value)
        }
    
    tts_process = await asyncio.create_subprocess_exec(sys.executable, "edgetts/tts.py", text, "Auto", '+0%', '+0%', 'Male', audio_path)
