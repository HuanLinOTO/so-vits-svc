...
class Unit2Mel(nn.Module):
    def __init__(
        ...
        self.decoder = GaussianDiffusion(
            out_dims, n_layers, n_chans, n_hidden, self.timesteps, self.k_step_max
        )
    ...
    def forward(self, units, mel2ph, f0, volume, g=None):
        ...
        x = (
            ...
            + self.f0_embed((1 + f0.unsqueeze(-1) / 700).log())
            + self.volume_embed(volume.unsqueeze(-1))
        )
    ...
    def OnnxExport(
        ...
        self.decoder.OnnxExport(
            project_name,
            init_noise=init_noise,
            export_denoise=export_denoise,
    ...
    def ExportOnnx(self, project_name=None):
        ...
        torch.onnx.export(
            ...
            input_names=["hubert", "mel2ph", "f0", "volume", "spk_mix"],
            output_names=["mel_pred"],
            dynamic_axes={"hubert": [1], "f0": [1], "volume": [1], "mel2ph": [1]},
            opset_version=16,
        )

        condition = torch.randn(1, self.decoder.n_hidden, n_frames)
        noise = torch.randn(
            (1, 1, self.decoder.mel_bins, condition.shape[2]), dtype=torch.float32
        )
