...
from SoVITS.spkmix import spk_mix_map
...
def main():
    ...
    # 一定要设置的部分
    parser.add_argument(
        "-m", "--model_path", type=str, default="logs/44k/{lastest}", help="模型路径"
    )
    parser.add_argument(
        "-c",
        "--config_path",
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=[f for f in os.listdir("raw") if isWavFile(f)],
        help="wav文件名列表，放在raw文件夹下且后缀为.wav",
    )
    parser.add_argument(
        "-cl",
        "--clip",
